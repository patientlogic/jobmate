...
# JobMate Release & Docker Build Script
...
BUMP_TYPE="${1:-patch}"
REGISTRY="ghcr.io"
IMAGE_NAME="patientlogic/jobmate"
...
echo "$GITHUB_TOKEN" | docker login "$REGISTRY" -u patientlogic --password-stdin
echo ""
...
docker buildx inspect jobmate-builder >/dev/null 2>&1 || \
  docker buildx create --name jobmate-builder --use

docker buildx use jobmate-builder
